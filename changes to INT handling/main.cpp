...
void playOrPause()
{
	...
	xQueueSendFromISR(buttonQueue, &playPause, NULL);
...
void back() 
{
	...
	int back = 1;
	NVIC_EnableIRQ(EINT3_IRQn);
	xQueueSendFromISR(buttonQueue, &back, NULL);
	portYIELD_FROM_ISR(yield);
...
void select()
{
	...
	int select = 2;
	NVIC_EnableIRQ(EINT3_IRQn);
	xQueueSendFromISR(buttonQueue, &select, NULL);
	portYIELD_FROM_ISR(yield);	
...
void down()
{
	...
	int down = 3;
	NVIC_EnableIRQ(EINT3_IRQn);
	xQueueSendFromISR(buttonQueue, &down, NULL);
	portYIELD_FROM_ISR(yield);
...
int main (void)
{
	...
	buttonQueue = xQueueCreate(3, sizeof(int));
    intr->Initialize();
