...
void sparkfunLCD::displayCurrentState()
{
    switch(currentState)
    {
		...
		case songsScreen:
		{
            ...
            spiSendString((char*)currentSongDirectory->at(songs_selectedOption).fileName.c_str());
            break;
		}
		...
		case nowPlayingScreen:
		{
            ...
            if(nowPlaying_selectedOption == 0)
            {
                ...
                nowPlaying_selectedString = playingSongDirectory->at(currentPlayingSong).fileName;
            }
            else
            {
